...
@Component({
  selector: 'app-finalized-appointments-per-lapse-chart',
  templateUrl: './finalized-appointments-per-lapse-chart.component.html',
  styleUrls: ['./finalized-appointments-per-lapse-chart.component.css']
})
export class FinalizedAppointmentsPerLapseChartComponent implements OnInit {
  ...
  onChange() {
    ...
    if (this.specialist && this.min && this.max) {
      ...
      this.appoService.getAllDateDesc().subscribe(app => {
        ...
        this.appointments = app.filter(a => a.patient && a.status === 'finalized');
