...
@Component({
  selector: 'app-finalized-appointments-per-lapse-chart2',
  templateUrl: './finalized-appointments-per-lapse-chart2.component.html',
  styleUrls: ['./finalized-appointments-per-lapse-chart2.component.css']
})
export class FinalizedAppointmentsPerLapseChart2Component implements OnInit {
  ...
  @Output() canvas4: EventEmitter<string> = new EventEmitter<string>();
    ...
    if (this._maxDate && this._minDate) {
      ...
      this.appoService.getAllDateDesc().subscribe(res => {
        ...
        res.map(a => {
          ...
          if (a.patient && a.status === 'finalized' && a.date < this._maxDate! && a.date > this._minDate!) {
            // if (a.date > this._maxDate!) {
            //   this._maxDate! = a.date;
            //   this._maxDate!.setHours(0, 0, 0, 0);
            // }
            // if (a.date < this._minDate!) {
            //   this._minDate! = a.date;
